...
import { generateTransactionSchema } from "@/schemas/transaction-schema";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validated = await validate(generateTransactionSchema, data);
    if (!validated.success) return response(400, validated.error);

    const trash = validated.data;
